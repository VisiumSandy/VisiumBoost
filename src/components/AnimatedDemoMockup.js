...
// All nav items (order must match real sidebar)
const NAV_ITEMS = [
...
// Cycle through sequential nav items (always going downward)
// Each entry: id of page to SHOW. Cursor points to the item BELOW it (next in cycle).
const CYCLE = ["dashboard", "clients", "wheel", "affiches", "codes"];
...
export default function AnimatedDemoMockup({ scale = 1 }) {
  ...
  const [cursor,   setCursor]   = useState({ x: 24, y: 200 });

  const navRefs = useRef([]);  // ref[i] → DOM node of NAV_ITEMS[i]
  const rootRef = useRef(null);

  const page    = CYCLE[pageIdx];
  // The cursor should point to the tab BELOW the current one (next in cycle)
  const nextIdx = (pageIdx + 1) % CYCLE.length;
  const nextPage    = CYCLE[nextIdx];
  ...
  const moveCursor = useCallback(() => {
    ...
    // Compensate for CSS scale: divide by scale to get inner coords
    const x = (er.left - rr.left) / scale + (er.width  / scale) * 0.38;
    const y = (er.top  - rr.top)  / scale + (er.height / scale) * 0.5;
    setCursor({ x, y });
  }, [nextNavIdx, scale]);

  useEffect(() => {
    // Move cursor to next tab after a short delay, then click
    const t1 = setTimeout(moveCursor, 400);
    const t2 = setTimeout(() => {
      setClicking(true);
      setTimeout(() => {
        setClicking(false);
            ...
            <aside style={{ width: 168, background: T.sidebar, flexShrink: 0, display: "flex", flexDirection: "column", borderRight: "1px solid rgba(255,255,255,0.05)" }}>
              ...
              <nav style={{ flex: 1, padding: "2px 6px", display: "flex", flexDirection: "column", gap: 1, overflow: "hidden" }}>
                {NAV_ITEMS.map((item, idx) => {
                  ...
                  const isNextTarget    = item.id === nextPage;
                  const isBeingClicked  = clicking && isNextTarget;
                  return (
                    <motion.div key={item.id}
                      ref={el => { navRefs.current[idx] = el; }}
                      animate={isBeingClicked ? { scale: [1, 0.93, 1] } : { scale: 1 }}
                      transition={{ duration: 0.28 }}
                      style={{
                        display: "flex", alignItems: "center", gap: 7,
                        padding: "6px 7px", borderRadius: 8,
                        background: active ? T.activeBg : isNextTarget ? "rgba(255,255,255,0.04)" : "transparent",
